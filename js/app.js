...
async function loadData() {
  ...
  try {
    ...
    console.warn('CDN coastlines unavailable, using local fallback:', e.message);
    try {
...
const mapModule = (() => {
  ...
  function buildCells() {
    ...
    for (let i = 0; i < n_lat; i++) {
      for (let j = 0; j < n_lon; j++) {
        ...
        if (projected.some(p => !p || isNaN(p[0]) || isNaN(p[1]))) {
          cells.push({ latIdx: i, lonIdx: j, lat, lon, idx: i * n_lon + j, d: null });
          continue;
        }
        ...
        if (xRange > 200) {
          ...
          d = `M${projected[0][0].toFixed(2)},${projected[0][1].toFixed(2)}` +
    ...
    cellsSel = gCells.selectAll('path.map-cell')
      .data(cells.filter(c => c.d), d => d.idx);
...
function showTooltip(event, d) {
  ...
  if (left + tipRect.width > wrapRect.width - 8) left = event.clientX - wrapRect.left - tipRect.width - 14;
  if (top + tipRect.height > wrapRect.height - 8) top = event.clientY - wrapRect.top - tipRect.height - 14;
...
const cellChartModule = (() => {
  ...
  function update() {
    ...
    g.selectAll('.cell-line, .cell-area, .threshold-line, .threshold-label, .crossing-dot, .crossing-text, .empty-msg').remove();
    ...
    if (state.selectedCell) {
      ...
      label = `${Math.abs(lat).toFixed(1)}°${lat >= 0 ? 'N' : 'S'}, ${Math.abs(lon).toFixed(1)}°${lon >= 0 ? 'E' : 'W'}`;
      source = 'cell';
    ...
    if (source) {
      ...
      d3.select('#cell-sub').text(`${SCENARIO_LABELS[state.scenario]} · annual mean anomaly ${source === 'region' ? '(regional area-weighted)' : '(grid cell)'}`);
    } else {
...
const histogramModule = (() => {
  ...
  function update() {
    ...
    const binner = d3.bin().domain([2015, 2105]).thresholds(d3.range(2015, 2105, 5));
    const bins = binner(crossed);

    // Y scale
    y = d3.scaleLinear().domain([0, d3.max(bins, b => b.length) || 1]).range([height - m.bottom, m.top]);
...
function updateStats() {
  ...
  if (state.mode === 'crossing') {
    d3.select('#map-title').text(`First year each region crosses +${state.threshold}°C`);
    d3.select('#map-sub').text(`Under ${SCENARIO_LABELS[state.scenario]} (${SCENARIO_DESC[state.scenario]}) · relative to 2015–2034 baseline · click any cell to inspect`);
  } else {
    d3.select('#map-title').text(`Temperature anomaly in ${state.year}`);
    d3.select('#map-sub').text(`Under ${SCENARIO_LABELS[state.scenario]} · °C above 2015–2034 baseline`);
  }
...
function updateSegActive(selector, activeBtn) {
  document.querySelectorAll(`${selector} .seg-btn`).forEach(b => b.classList.toggle('active', b === activeBtn));
}
...
async function main() {
  try {
    ...
    document.getElementById('map-loading').textContent = `error: ${err.message}`;
  }
}
